...
- (void)viewDidLoad {
    ...
    comment.text = @"VFinance沙箱支付不产生真实交易";
...
- (UIImage *)parseChannel:(PayChannel)channel {
    ...
    if (channel == PayChannelUnApp || channel == PayChannelVFApp) {
...
- (UIImage *)imagesNamedFromCustomBundle:(NSString *)imgName {
    NSString *bundlePath = [[NSBundle mainBundle].resourcePath stringByAppendingPathComponent:@"SaasWalletSDK.bundle"];
...
- (void)back {
    resultCode = VFErrCodeUserCancel;
    resultMsg = @"支付取消";
...
- (void)pay {
    ...
    NSString *host = [NSString stringWithFormat:@"%@%@/%@", [VFPayUtil getBestHostWithFormat:kRestApiSandboxNotify], [VFPayCache sharedInstance].appId, [VFPayCache sharedInstance].vfResp.vfId];
    NSLog(@"sandboxPay id = %@", [VFPayCache sharedInstance].vfResp.vfId);
...
- (void)doNotifyResponse:(NSDictionary *)response {
    [loading stopAnimating];
    if ([response integerValueForKey:kKeyResponseResultCode defaultValue:VFErrCodeCommon] == VFErrCodeSuccess) {
        resultMsg = @"支付成功";
        resultCode = VFErrCodeSuccess;
    }
    [self dismissViewControllerAnimated:YES completion:^{
